    ...
    if upload_data is not None:
        ...
        if st.sidebar.checkbox("プロット範囲 設定"):
            max_x = st.sidebar.number_input(
                'プロット上限範囲　共通設定', value=max_x)
            min_x = st.sidebar.number_input(
                'プロット下限範囲　共通設定', value=min_x)
        ...
        if st.sidebar.checkbox("その他プロット詳細設定"):
            ...
            # 色
            sl_color = st.sidebar.selectbox(
                '線色', [sl_color, "green", "tab:blue"])
            # 線タイプ
            sl_style = st.sidebar.selectbox(
                '線タイプ', [sl_style, '--', '-.', ':'])
            # 線幅
            ...
            al_color = st.sidebar.selectbox(
                '線色　　', [al_color, "tab:blue", "red"])
            # 線タイプ
            al_style = st.sidebar.selectbox(
                '線タイプ　　', [al_style, '-', '-.', ':'])
            # 線幅
        ...
        def pci_fig(column_name, column_data, usl, lsl, max_x, min_x):
            ...
            if sl_setting == "sl_setting_yes":
                usl = st.number_input(
                    str([column_name]) + '上限規格値　個別設定', value=usl)
                lsl = st.number_input(
                    str([column_name]) + '下限規格値　個別設定', value=lsl)
            # プロット範囲設定
